...
def command(message, res=False):
    ...
    bot.send_sticker(chat_id, 'CAACAgIAAxkBAAIF42KLquh3CMA3CNIpRHMdoBwvFbfcAAKKEQACSvbIS3E2ed2tuM6SJAQ')
    txt_message = f"Привет, {message.from_user.first_name}! Я тестовый бот " \
                  f"для курса программирования на языке Python. Чем мы займемся сегодня?"
...
def get_text_messages(message):
    ...
    if cur_menu is not None and ms_text in cur_menu.buttons:  # проверим, что команда относится к текущему меню
        ...
        if module != "":  # проверим, есть ли обработчик для этого пункта меню в другом модуле,
            # если да - вызовем его (принцип инкапсуляции)
...
def send_help(bot, chat_id):
    ...
    btn1 = types.InlineKeyboardButton(text="Напишите автору", url="https://t.me/Windesmoke")
    markup.add(btn1)
    img = open('_OSbixmRjQ4.jpg', 'rb')
